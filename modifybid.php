...
<div class="navbar" id="menu">
  ...
  <a href="index1.php" style="background-color: #45a049;">Signout</a>
   ...
   <a href="modifybid.php" style="color:orange;">Modify Bid</a>
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
               if($row[approval]==$yes){
                   if($row[startdate]<=$todaydate){
                                      if($row[sellerid]==$sellerid1)    {        
...
?>
 ...
 <form action="modmod.php" method="post" >
  <input type="hidden" name="itemid" value=<?php echo $roll ?> >
  <input type="submit" value="modify Bid">
</form>
</center>
</div>
<?php
